...
if not path.exists('femaleClfLinRes.obj'):
    ...
    with open('femaleClfLinRes.obj', 'wb') as f:
        pickle.dump(clf, f)
else:
    print('clf exists. Loading...')
    with open('femaleClfLinRes.obj', 'rb') as f:
        clf = pickle.load(f)
...
for root, dirs, files in list(walk("../femaleTest/"))[8:]:
    if len(files) == 0:
        continue
    print(root)
