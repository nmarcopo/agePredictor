...
def get_prediction(classifier, img_path, debug=False):
 ...
 #   print(conv_base.summary())
    ...
    for label in labels:
        age = int(img_path.split('_')[0].split('/')[-1])
        if age in range(int(label.split('-')[0]),int(label.split('-')[1])+1):
            truth = label
    predictionList= []
    predictionText = labels[int(prediction)]
    print('image put in label: ',truth)
...
if not path.exists('femaleClf.obj'):
    ...
    svm = svm.SVC(degree=3,kernel='poly')  # As in Tang (2013)
    clf = GridSearchCV(svm, param, cv=10)
    clf.fit(X_train, y_train)

    with open('femaleClf.obj', 'wb') as f:
        pickle.dump(clf, f)
else:
    print('clf exists. Loading...')
    with open('femaleClf.obj', 'rb') as f:
        clf = pickle.load(f)
...
for root, dirs, files in list(walk("../maleTest/"))[9:]:
    if len(files) == 0:
        continue
    print(root)
